...
const Dot = memo(function Dot({ slide, index, isActive, onSelect }: DotProps) {
  ...
  return (
    <button
      ...
      onClick={handleClick}
      className="flex items-center justify-center p-3 -m-3 cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-[#c4977a] rounded-full"
    >
      <span
        aria-hidden="true"
        className="block rounded-full transition-all duration-300 ease-out hover:opacity-75"
        style={{
          width: isActive ? 28 : 8,
          height: 8,
